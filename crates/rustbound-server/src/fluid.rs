...
//! treats them as non-solid so players can enter them. The client owns swim
//! physics; the server must not rewrite Y every tick or it will spam
//! Synchronize Player Position and lock the camera / trigger "loading terrain".
//! Lava still deals periodic contact damage on the server.
...
mod tests {
    ...
    #[test]
    fn water_range_does_not_claim_netherrack_palette() {
        // Historical bug: netherrack was wrongly coded as 87 (a water level).
        assert!(!is_water(5850));
        assert!(!is_fluid(5850));
        assert!(!is_fluid(7415));
    }
}
